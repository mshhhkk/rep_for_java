...
public class GraphicsDisplay extends JPanel {
    ...
    protected Point2D.Double translateXYtoPoint(double x, double y)//преобразование координат из системы данных (x, y) в экранные координаты на панели отображения.
    {
    ...
    protected double[] translatePointToXY(int x, int y) //переводит координаты экрана (x, y) обратно в координаты данных
    {
        return new double[]{this.viewport[0][0] + (double)x / this.scaleX, this.viewport[0][1] - (double)y / this.scaleY};
    }

    protected int findSelectedPoint(int x, int y) //поиск ближайшей точки данных, которая находится вблизи указанных экранных координат (x, y)
    {
    ...
    public class MouseHandler extends MouseAdapter { public MouseHandler() { }

        public void mouseClicked(MouseEvent ev) {
            if (ev.getButton() == 3) //проверяет, была ли нажата правая кнопка мыши
            {
        ...
        public void mousePressed(MouseEvent ev) //метод подготавливает компонент графика для дальнейших действий в зависимости от режима работы
        {
            if (ev.getButton() == 1) {
        ...
        public void mouseReleased(MouseEvent ev) {
            if (ev.getButton() == 1) {
                GraphicsDisplay.this.setCursor(Cursor.getPredefinedCursor(0));//Устанавливает стандартный курсор мыши (стрелку) после отпускания кнопки
            ...
            }//завершение действий, связанных с изменением точек на графике или масштабированием области после отпускания левой кнопки мыши
        }
    }
    ...
    public class MouseMotionHandler implements MouseMotionListener {
        ...
        public void mouseMoved(MouseEvent ev)//движения без зажатия
        {
            GraphicsDisplay.this.selectedMarker = GraphicsDisplay.this.findSelectedPoint(ev.getX(), ev.getY());
            if (GraphicsDisplay.this.selectedMarker >= 0) //указатель находится над точкой графика
            {
                GraphicsDisplay.this.setCursor(Cursor.getPredefinedCursor(8));//курсор стрелочки в обе стороны
            } else {
                GraphicsDisplay.this.setCursor(Cursor.getPredefinedCursor(0));//обычный курсор
            }

            GraphicsDisplay.this.repaint();
        }

        public void mouseDragged(MouseEvent ev) //обрабатывает событие перемещения мыши с зажатой левой кнопкой
        {
            if (GraphicsDisplay.this.changeMode) {
